...
- **ORM** : Drizzle ORM connecté à PostgreSQL via Neon serverless.
...
│   ├── lib/                 # Services partagés (auth, OTP, Drizzle, Intouch, notifications, rate limit)
...
- **Migrations** : `drizzle-kit generate` / `drizzle-kit migrate` pour versionner et appliquer les évolutions de schéma.
- **Environnements** :
  - Dev : local (Bun + Next dev, PostgreSQL local ou Neon).
...
La plateforme Sama Naffa repose sur une architecture Next.js moderne, orchestrant un front réactif et un backend monolithique structuré. Drizzle ORM et PostgreSQL fournissent le socle actif pour les données clients et transactionnelles, avec quelques points de compatibilité legacy encore présents pour l’ancienne API Prisma. Des intégrations avec Intouch, Twilio et Mailgun assurent la continuité des parcours financiers. Les priorités à court terme portent sur l’industrialisation de la sécurité (chiffrement, rotation des secrets), l’observabilité, et l’extension des fonctionnalités du portail client. Ce document au format racine sert de référence pour onboarder les équipes techniques et aligner les décisions futures.
