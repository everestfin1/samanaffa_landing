...
- `vite.config.ts` and TanStack Start are wired.
- `src/app/__root.tsx` and `src/app/index.tsx` are migrated to TanStack file routes.
- `src/routeTree.gen.ts` is generated by TanStack Router during `bun run dev`.
...
TanStack Start middleware requires explicit config in `vite.config.ts`:

```ts
tanstackStart({
  srcDirectory: 'src',
  start: {
    configFile: 'src/start.ts',  // Required for middleware
  },
  router: { ... },
})
```

The `src/start.ts` file registers request middleware:
...
**Note**: Middleware only runs on SSR. For client-side navigation, use `beforeLoad` guards in routes (see `src/app/__root.tsx`).

### Temporary Router Generator Ignore Rules

During the migration, this repo contains legacy Next.js App Router files under `src/app/**` (e.g. `page.tsx`, `route.ts`, and `src/app/api/**`). TanStack Router’s generator will try to parse all route files under `src/app` unless we ignore them.

Temporary configuration (until all routes are migrated):
...
If the project currently uses NextAuth via `src/app/api/auth/[...nextauth]/route.ts` and `getServerSession`, these will be replaced.
...
**Handler signature** (Vercel Functions):
...
Notes:
- `@vercel/node` is still supported for Vercel Functions. If you see warnings, use the latest `@vercel/node` version and check the Vercel docs for runtime updates.
- TanStack Start server routes are optional in this setup; you can keep all backend APIs in `/api` and point your frontend fetch calls there.
