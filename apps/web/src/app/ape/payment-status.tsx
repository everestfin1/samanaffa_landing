...
function PaymentStatusContent() {
  ...
  const referenceNumber = searchParams.referenceNumber || searchParams.num_command;
  const amount = searchParams.amount;
  const transactionId = searchParams.transactionId || searchParams.num_transaction_from_gu;
  const source = searchParams.source || 'intouch';
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) {
          clearInterval(timer);
          (navigate as any)({ to: '/apesenegal/payment-status' });
          return 0;
        }
        return prev - 1;
      });
...
export const Route = createFileRoute('/ape/payment-status')({
